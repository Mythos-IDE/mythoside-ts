...
What's untouched by the reset: `src/services/cryptoService.ts` (SHA-256 password hashing via Web Crypto) and `src/services/firebase.ts` (a configured but **unused** Firebase app/Firestore/Auth client — not imported anywhere in `src/`; local-first still holds).

**File I/O and watching are a Rust responsibility, not TypeScript** — decided explicitly, not the default Tauri-template split. Parsing/validation (what a Scene _is_) stays in TS (`src/core/manuscriptFormat.ts`, pure string transforms, no native capability needed); reading/writing/watching the filesystem (what a native OS actually gives you) is Rust's job. See `src-tauri/src/watcher.rs` for the first real implementation of this split.
...
- Not implemented yet: walking/reading an actual project directory into these types, wiring the Rust watcher's change events to trigger a re-parse on the TS side, and the SQLite/FTS5 background index.
